...
export function Board({ board, setBoard, rows: numRows, columns: numColumns, gameState, increaseNumRevealed, increaseFlagsPlaced, startTimer, loseGame }) {
	...
	return (
		<div className="Board m-auto">
			{board.map((row, index) => {
				return (
					<div key={`cell-row-${index}`} className="board-row d-flex justify-content-center">
						{row.map((cell) => {
							return <Cell key={`cell-coord-${cell.x}-${cell.y}`} {...cell} toggleFlag={toggleFlag} revealCell={revealCell} gameState={gameState} numRows={numRows} numColumns={numColumns} />;
						})}
					</div>
				);
			})}
